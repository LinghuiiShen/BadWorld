...
class FirstFramePGDAttack:
    ...
    def build_conditional_dict(
        ...
        # First generate global actions, then slice out the real-frame actions corresponding to the local window
...
#####Keep action as ZERO (no action)#########
        mouse_condition = torch.zeros_like(mouse_condition)
        keyboard_condition = torch.zeros_like(keyboard_condition)
    ...
    def run_attack_forward_once(
        ...
        total_length = actual_history_length + target_length

        # First-frame latent, used as proxy history
        history_lat = self.clean_first_lat.repeat(1, 1, history_length, 1, 1)
        ...
        # Use a high-noise timestep for the target;
        ...
        # ============================================================
        # loss:
        # Maximize the difference between pred_v and the ground-truth velocity that "returns to the first-frame latent"
        #
        # ground truth velocity:
        #   v_gt = x0(noisy target latent) - x1(context first-frame latent); gradient ascent
        #
        # Compute only on the target part here, ignoring the history proxy.
        # ============================================================
        flow_target = flow_pred[:, :, actual_history_length:]  # [1,16,T,44,80]

        target_x1 = self.clean_first_lat.repeat(1, 1, target_length, 1, 1)  # [1,16,T,44,80]
        gt_velocity_target = noisy_target-target_x1

        loss_flow = (flow_target.float() - gt_velocity_target.float()).pow(2).mean()
    ...
    def attack(self):
        ...
        for step in range(1, self.args.num_steps + 1):
            ...
            with torch.no_grad():
                # Maximize the objective: gradient ascent
...
def build_parser():
    ...
    # paths
    parser.add_argument("--input_image", type=str, default="demo_images/gta_drive/0001.png")
    parser.add_argument(
        "--config_path",
        type=str,
