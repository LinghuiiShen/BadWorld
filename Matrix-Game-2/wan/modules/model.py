...
# 当前视频 patch token 去 query：首帧图像的全局语义特征（context frame)(不变，forward一次后就被cache)
    def forward(self, x, context, crossattn_cache=None):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C]
        ...
        if crossattn_cache is not None:
            ...
            k = self.norm_k(self.k(context)).view(b, -1, n, d)
            v = self.v(context).view(b, -1, n, d)
        # print("cross attention q: ", q.shape)
        # print("cross attention k: ", k.shape)
        # print("cross attention v: ", v.shape)
        # compute attention
