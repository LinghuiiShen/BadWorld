...
class FirstFramePGDAttack:
    ...
    def build_conditional_dict(
        ...
        # Slice the window directly from the clean global cond to avoid re-encoding the long video at each step
        cond_concat = self.clean_cond_concat_global[:, :, window_start_frame: window_start_frame + total_length].clone()

        # Only use the adversarial first frame to replace the image latent corresponding to global frame 0
        adv_first_lat = self.encode_single_frame_to_latent(adv_first_frame)   # [1,16,1,44,80]

        # cond_concat channels: [mask(4), img_lat(16)]
        # If the current window covers global frame 0, replace that frame's image latent in the window
        ...
        # First generate global actions, then use the global action prefix up to the end of the current target block
        num_real_frames_global = 1 + 4 * (global_length - 1)
        cond_data = Bench_actions_gta_drive(num_real_frames_global)

        # Align with inference: pass the global action prefix up to the end of the current target block
    ...
    def sample_timestep_tensor(self, num_latent_frames: int) -> Tuple[torch.Tensor, int, float]:
        ...
        t_idx = random.randint(0,50) # high timesteps, denoising prior, low indexs
    ...
    def run_attack_forward_once(
        ...
        # block_idx=0: generate frames 0:2; block_idx=1: generate frames 3:5... (the adversarial first frame is not counted here)
        ...
        total_length = actual_history_length + target_length   # target_length for block 0; history + target otherwise

        # First-frame latent used as proxy history
        history_lat = self.clean_first_lat.repeat(1, 1, history_length, 1, 1)   # [1,16,1,44,80]

        # The current target block is still a noisy latent
        ...
        # sample_timestep_tensor returns [1, target_length]
        timestep = torch.ones(
            [1, total_length],
            device=self.device,
        ...
        # Optimize only the 3 target frames, not the history proxy
        ...
        aux_info = {
            ...
            # "current_start": current_start,
            # "x_input_shape": tuple(x_input.shape),
            # "flow_shape": tuple(flow_pred.shape),
            # "flow_target_shape": tuple(flow_target.shape),
            # "gt_velocity_shape": tuple(gt_velocity_target.shape),   # [MODIFIED]
            # "mouse_cond_shape": tuple(conditional_dict["mouse_cond"].shape),
            # "keyboard_cond_shape": tuple(conditional_dict["keyboard_cond"].shape),
            # "cond_concat_shape": tuple(conditional_dict["cond_concat"].shape),
            # "visual_context_shape": tuple(conditional_dict["visual_context"].shape),
        }
    ...
    def attack(self):
        ...
        # Precompute a maximum-length clean cond_concat, then slice it directly later
        ...
        for step in range(1, self.args.num_steps + 1):
            ...
            with torch.no_grad():
                # Minimize the objective: gradient descent
...
def build_parser():
    parser = argparse.ArgumentParser(
        "PGD attack on Matrix-Game-2 input first frame (GTA only, random target blocks)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of latent frames in each sampled target block; default is 3.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Unused in this version; kept for compatibility with earlier action-window experiments."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Randomly sample target block index from [0, max_block_idx]. block_idx=0 attacks frames 0:2; block_idx=1 attacks frames 3:5."
    )

# Note: The adversarial condition goes through a separate path and is not used as the first-frame input. It also has no corresponding action.
# In this model, the first frame is the first generated frame.
    # misc
