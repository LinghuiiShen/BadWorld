...
# Assuming there are 4 historical frames (0, 1, 2, 3) and we need to generate the next three frames (4, 5, 6),
# then the shape of the action sequence currently passed into the block is:
    # mouse_cond: [B, 25, 2] ------- -> 1 + 4 * (4 + 3 - 1) = 1 + 24 = 25, vae_time_compression_ratio = 4,  mouse_dim=keyboard_dim=2, 
    # keyboard_cond: [B, 25, ]
    if mode != 'templerun':
        ...
        for current_num_frames in all_num_frames:
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                if index < len(self.denoising_step_list) - 1:
                    ...
                    # flow_norm = flow_pred.float().pow(2).mean()
                    # print(f"[Inference] step={index} | t={current_timestep.item()} | flow_norm={flow_norm.item():.6f}")
                    print(f"[Inference] step={index} | t={current_timestep.item()} ")
                    import sys
                    sys.stdout.flush()
                    ...
                    flow_norm = flow_pred.float().pow(2).mean().sqrt()
                    print(f"[Inference] step={index} | t={current_timestep.item()} | flow_norm={flow_norm.item():.6f}")
                    import sys
                    sys.stdout.flush()
