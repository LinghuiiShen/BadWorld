...
# inference 每个去噪 step 都把 “要预测的 latent chunk(new_latents)” + “历史多尺度 clean_latents 前缀(FramePack)” + “按帧给的 camera embedding(含 mask)” + “文本 prompt embedding” 
# 一起喂给 WanModelMoe；模型先把所有 latent patchify 成 token，并把多尺度 clean token 拼到前面；
# camera embedding 被投影到 unified_dim 后复制到每个 token 上，作为 modality_inputs；每层 block 在 Self-Attn 前用 MoE（固定路由到对应专家）把 camera 条件注入到 token；然后正常做 Self-Attn(带 RoPE) + Cross-Attn(到文本) + FFN；最后只输出主 token 的预测结果，
# 回到 latent 空间，交给 scheduler 更新 new_latents。
...
def generate_sekai_camera_embeddings_sliding(
    ...
    """
    time_compression_ratio = 4 #latent/video 的时间步和原始相机数据不是一一对应的，而是按 4 帧压缩。
    ...
    #训练时：sekai 的 camera embedding 来自真实相机轨迹, inference时既支持真实Pose，干支持手工合成（1-7，一般用这个）
    if use_real_poses and cam_data is not None and 'extrinsic' in cam_data:
        print("🔧 Using real Sekai camera data")
        cam_extrinsic = cam_data['extrinsic'] # 一个 N x 4 x 4 的外参矩阵序列
        ...
        for i in range(max_needed_frames):
            ...
            if next_frame_idx < len(cam_extrinsic):
                ...
                relative_pose = compute_relative_pose(cam_prev, cam_next)#算相对位姿
                ##更关心的是**“从这一帧到下一帧相机怎么动”**而不是相机在全局世界坐标中的绝对位置
                relative_poses.append(torch.as_tensor(relative_pose[:3, :]))#取前三行，得到一个 3x4 矩阵。
            else:
        ...
        pose_embedding = torch.stack(relative_poses, dim=0)#每帧就有 12 维 pose 表示 [M, 3, 4]
        pose_embedding = rearrange(pose_embedding, 'b c d -> b (c d)')#每帧就有 12 维 pose 表示。 [M, 12]
    ...
    else:#合成相机轨迹,不是从真实外参里算，而是手工定义每一帧的相对运动矩阵。
        # Ensure generating a sufficiently long camera sequence
        max_needed_frames = max(
            start_frame + initial_condition_frames + new_frames, 
            framepack_needed_frames, 
        ...
        CONDITION_FRAMES = initial_condition_frames #历史条件帧
        STAGE_1 = new_frames//2
        STAGE_2 = new_frames - STAGE_1 #后面的生成部分，再分成两个阶段
        
        if cam_type==1:
            print("--------------- FORWARD MODE ---------------")
            relative_poses = []
            for i in range(max_needed_frames):  #对于每一帧，都加一个3*4+1的relative camera emb
                if i < CONDITION_FRAMES: #前 CONDITION_FRAMES 都是“零运动”
                    # Input condition frames default to zero motion camera pose
                    pose = np.eye(4, dtype=np.float32)
                elif i < CONDITION_FRAMES+STAGE_1+STAGE_2:
                    # Forward
                    forward_speed = 0.03
                    pose = np.eye(4, dtype=np.float32) #每帧沿 z 方向前进一点点，速度 0.03
            ...
            for i in range(max_needed_frames):
                if i < CONDITION_FRAMES:
                    ...
                    pose = np.eye(4, dtype=np.float32)
                #标准绕 y 轴的 yaw 旋转矩阵（只保留 x-z 平面部分）
                    # 每帧向左转一点
                    # 不前进
                    pose[0, 0] = cos_yaw
                    pose[0, 2] = sin_yaw
            ...
            print("--------------- S CURVE MODE ---------------")
            #前半段左转前进，后半段右转前进，还加了一点侧向漂移，所以轨迹更像一个 S 弯。
            relative_poses = []
            for i in range(max_needed_frames):
                if i < CONDITION_FRAMES:
                    # Input condition frames default to zero motion camera pose
                    ...
                    if i < CONDITION_FRAMES+STAGE_1+STAGE_2//3: #在右转阶段开始的前 1/3 段，除了“右转 + 前进”之外，还额外做一点横向平移
                        radius_shift = -0.01
                    else:
                        radius_shift = 0.00
            ...
            for i in range(max_needed_frames):
                ...
                if i >= CONDITION_FRAMES and i < CONDITION_FRAMES + STAGE_1 + STAGE_2:
                    shift = 0.02  # 横向速度
                    pose[0, 3] = shift  # x方向平移

                relative_poses.append(torch.as_tensor(pose[:3, :]))

        elif cam_type == 12:
            print("--------------- SMOOTH ZOOM IN ---------------")
            relative_poses = []
            total = max(1, STAGE_1 + STAGE_2)

            for i in range(max_needed_frames):
                pose = np.eye(4, dtype=np.float32)

                if i >= CONDITION_FRAMES and i < CONDITION_FRAMES + total:
                    t = (i - CONDITION_FRAMES) / total
                    forward_speed = 0.01 + 0.04 * t**2  # 二次加速
...
def prepare_framepack_sliding_window_with_camera_moe( #camera_embedding_full要和当前 sliding window 的 latent/history 结构对齐
    ...
    clean_latents_4x = clean_latents_combined[:, 1:17, :, :] #20帧里前面的帧。如果history不满20帧，会用0占位
    clean_latents_2x = clean_latents_combined[:, 17:19, :, :] #倒数第2，3帧
    clean_latents_1x = clean_latents_combined[:, 19:20, :, :] #最后一帧
    #不满 20 帧时，会把已有历史帧右对齐填进一个固定长度 20 的缓存里，前面空出来的位置用 0 latent 占位；同时用 -1 index 标记这些占位是无效历史。
    clean_latents = torch.cat([start_latent, clean_latents_1x], dim=1)
    
    # Process cam_emb 把相机条件和 latent 时间轴对齐
    actual_needed_frames = T + target_frames_to_generate
    if camera_embedding_full.shape[0] < actual_needed_frames:#如果 camera 序列不够长，就补
    ...
    # Select corresponding part from complete camera sequence取出当前窗口所需的 camera 序列
    ...
    return {
        ...
        'camera_embedding': combined_camera, #形状是[actual_needed_frames, 13]
...
def inference_moe_framepack_sliding_window(
    ...
    #         # ===== metrics: before scheduler step =====
    #         latent_rms = tensor_rms(new_latents)
    #         pred_rms = tensor_rms(noise_pred)
    #         pred_hf_ratio = laplacian_hf_ratio(noise_pred)
    #         pred_latent_cos = cosine_similarity_flat(noise_pred, new_latents) #模型输出方向 vs 当前状态方向 的关系, cos大：觉得当前latent就是主要噪声, cos小：觉得当前latent已经去噪得差不多了

    #         # 先真正走一步，再看 step delta（更适合 velocity / flow）
    #         new_latents_next = pipe.scheduler.step(noise_pred, timestep, new_latents)
    #         step_delta = new_latents_next - new_latents
    #         step_delta_rms = tensor_rms(step_delta)

    #         print(
    #             f"  [denoise {i+1:02d}/{len(timesteps)}] "
    #             f"t={float(timestep):8.3f} | "
    #             f"latent_rms={latent_rms:.6f} | "
    #             f"pred_rms={pred_rms:.6f} | "
    #             f"pred_hf_ratio={pred_hf_ratio:.6f} | "
    #             f"pred_latent_cos={pred_latent_cos:.6f} | "
    #             f"step_delta_rms={step_delta_rms:.6f}"
    #         )

    #         new_latents = new_latents_next
...
#-------------------
    parser.add_argument("--condition_image",
                        type=str,
                        default="0to50_0001.png",
                        # default="../examples/condition_images/garden_1.png",
                        help="Input image for novel view synthesis.")

    parser.add_argument("--prompt", 
                    type=str, 
                    default="Driving on the highway, the sky is a light purple color and the view is very expansive.",
                    help="text prompt for video generation")
    parser.add_argument("--cam_type", type=str, default="4", help="Camera type for video trajectory")
    parser.add_argument("--total_frames_to_generate", type=int, default=32)
    parser.add_argument("--output_path", type=str, 
                    #    default='../outputs/j1/garden1_4.mp4')
                        default='./outputs/j1/garden1_4.mp4')
