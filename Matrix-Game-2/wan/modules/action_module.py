...
class ActionModule(nn.Module):
    ...
    def forward(self, x, tt, th, tw, mouse_condition=None, keyboard_condition=None, block_mask_mouse=None, block_mask_keyboard=None, is_causal=False, kv_cache_mouse=None, kv_cache_keyboard=None, start_frame=0, use_rope_keyboard=True, num_frame_per_block=3, force_target_only=False):
        '''
...
######################### MOUSE ###########################        
        if self.enable_mouse and mouse_condition is not None:
            ...
            if force_target_only:
                hidden_states = hidden_states[:, -num_frame_per_block:, :]
            #####
            B, N_frames, C = mouse_condition.shape  #每个空间位置 S，单独拿出它 across time 的 token 序列
        else:
            hidden_states = x
        # padding
# 一个 latent frame 会看：
#     最近 3 个 latent-frame 窗口
#     总共 3 * 4 = 12 个 mouse action steps
        pad_t = self.vae_time_compression_ratio * self.windows_size   #4*3
        if self.enable_mouse and mouse_condition is not None:  
            #pad: 把 action 的第一个 step 复制 12 次，接到最前面。
            pad = mouse_condition[:, 0:1, :].expand(-1, pad_t, -1) #如果当前显式传入的是 [1, n(比如25，history+target所有的action序列), 2]，那么先 pad 12 个 step，变成[1, n+12, 2],给最早的几个 latent frame 也凑够一个“3-frame action window”
            mouse_condition = torch.cat([pad, mouse_condition], dim=1)
            # print("mouse condition shape: ", mouse_condition.shape)
            # if is_causal and kv_cache_mouse is not None:
            if (is_causal and kv_cache_mouse is not None) or force_target_only: 
            ...
            # time_len = num_frame_per_block if (is_causal and kv_cache_mouse is not None) else N_feats
            # group_mouse = group_mouse.unsqueeze(-1).expand(B, time_len, pad_t, C, S)
            group_mouse = group_mouse.permute(0, 4, 1, 2, 3).reshape(B * S, num_frame_per_block, pad_t * C) 
            # print("group mouse shape: ", group_mouse.shape) # --> [880, 3, 1536 + 24]
            # 当前空间位置的视觉 token（hidden_states） 和  对应时间窗口内的 mouse 控制
            # 先拼接，再过 MLP。
            group_mouse = torch.cat([hidden_states, group_mouse], dim = -1) #拼接 mouse window ---> [880, 3, 1536 + 24]
            group_mouse = self.mouse_mlp(group_mouse) #变回 mouse hidden dim 比如1024
            # qkv
            # Q = 来自“视觉 token + mouse window”融合后的特征
            # K = 同样来自这个融合特征
            # V = 同样来自这个融合特征
            mouse_qkv = self.t_qkv(group_mouse) #mouse 不是独立做 K/V 后让视频去 attend，它是先和视觉状态融合一下，再产生 q/k/v。
            ...
            # 先做一条独立 attention 分支，得到 mouse-conditioned feature，再通过 proj_mouse 投回主维度，最后 残差加回视觉 token。
...
#################### KEYBOARD #############################
        ...
        if self.enable_keyboard and keyboard_condition is not None:
            ...
            # print("keyboard condition shape: ", keyboard_condition.shape)
            # if is_causal and kv_cache_keyboard is not None:
            if (is_causal and kv_cache_keyboard is not None) or force_target_only:
                ...
                keyboard_condition = self.keyboard_embed(keyboard_condition) #先做 embedding， 把 one-hot / 离散动作先变成 hidden vector。
               # 同样先按时间窗口分组
            ...
            # print("group keyboard shape: ", group_keyboard.shape) # [1,3,12*]
            # apply cross attn  ： 视觉 token 去 attend keyboard action token
            mouse_q = self.mouse_attn_q(hidden_states)#其实是拿当前 hidden_states（也就是视觉 token）去生成 query。
            keyboard_kv = self.keyboard_attn_kv(group_keyboard)#key/value 来自 keyboard action window
            ...
            if use_rope_keyboard: 
                ...
                if is_causal:
                    if kv_cache_keyboard is None:
                        ...
                        # keyboard 是：
                            # Q 来自视觉 token
                            # K/V 来自 keyboard action token
                            # 本质上是 cross-attention 式条件注入
        ...
        #keyboard 分支也是：做完 action-conditioned attention 后，再 residual 加回视觉 token
...
# mouse / camera
# 是 attention 进去的，更准确地说：
# 先把视觉 token 和 mouse 时间窗口拼起来
# 用这个融合后的序列做一层 self-attention
# 再投影回视觉空间
# 残差加回去
# 所以 mouse 是：
# fused self-attention injection
# 不是纯加法。

# keyboard
# 也是 attention 进去的，更准确地说：
# query 来自视觉 token
# key/value 来自 keyboard token
# 做 cross-attention
# 再投影回主干
# 所以 keyboard 是：
# cross-attention-style injection
# 也不是纯加法。