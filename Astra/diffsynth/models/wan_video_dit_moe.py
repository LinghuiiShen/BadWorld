...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        # print("cross attn x: ", x.shape)
        # print("cross attn y: ", y.shape)
        ...
        v = self.v(ctx)
        # print("cross attn q: ", q.shape)
        # print("cross attn k: ", k.shape)
        # print("cross attn v: ", v.shape)
        x = self.attn(q, k, v)
        # print("cross attn x: ", x.shape)
        if self.has_image_input:
            print("Has image input, attention to img")
...
class ModalityProcessor(nn.Module):   #比如sekai 的 13 维输入，投到统一维度 25
    """模态处理器 - 将不同模态投影到统一维度"""
...
class DiTBlockWithMoE(nn.Module):
    ...
    def forward(self, x, context, cam_emb, t_mod, freqs, 
        ...
        if self.use_moe and modality_inputs and hasattr(self, 'moe') and router_weights is not None:
            ...
            # action / modality embedding 先经过 MoE experts
            # MoE 输出和 transformer token 相加
            # 再送进 self-attention            
            if combined_modality_input is not None:
                # 🔧 使用全局router的权重和索引

                # print("combined_modality_input: ", combined_modality_input.shape)
                 #torch.Size([1, 15600, 25])
                # 1：batch size
                # 15600：当前这一轮 transformer 里，总共有 15600 个 token 位置
                # 25：每个 token 上附着了一个 25 维的 modality/action embedding

                #[1,L,25]→[1,L,1536] 把动作/相机条件作为一个 residual bias，注入到每个 token 上。
                ...
                # torch.Size([1, 15600, 1536])
                # 每个 token 的动作条件，经过 expert 后，被变成可以直接加到 transformer token 上的 1536 维向量。
        ...
        # print("input_x_after_proj: ", input_x.shape)
...
class WanModelMoe(torch.nn.Module):
    ...
    def process_input_hidden_states(self, 
                                ...
                                clean_latents=None, clean_latent_indices=None, #2×30×52=3120, torch.Size([1, 3120, 1536])
                                clean_latents_2x=None, clean_latent_2x_indices=None,
                                clean_latents_4x=None, clean_latent_4x_indices=None,
                                cam_emb=None, #每帧一个 camera / action 向量，比如 sekai 是 [pose(12) + mask(1)]
                                modality_inputs: Optional[dict] = None):  # 🔧 新增modality_inputs参数 只是告诉后面这是哪种 modality，比如 sekai / nuscenes / openx
        """🔧 处理FramePack风格的多尺度输入 + MoE模态输入处理 - 完全照wan_video_dit_recam_future实现"""
        
        # 主要latents处理
        hidden_states, grid_size = self.patchify(latents) #当前要生成的8帧
        ...
        if cam_emb is not None:
            ...
            # 🔧 为主要latents处理camera空间扩展。#原来一帧只有一个 25 维条件向量，现在复制到该帧所有空间 patch 上，让这一帧里的每个 token 都带上同一个控制条件
    ...
    def forward(self, 
        ...
        modality_inputs, cam_emb = self.process_modality_inputs(modality_inputs)  #把原始 action/camera 序列做 projection。
        ...
        #“历史 chunk + 条件 + 当前 chunk”那种 autoregressive world generation 结构，真正翻译成 transformer token 序列。
        ...
        # print("processed_cam_emb: ", processed_cam_emb.shape)
        # print("processed_modality_inputs: ", processed_modality_inputs.shape)
