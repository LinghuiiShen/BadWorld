...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            total_num_frames = grid_sizes[0].item()
            ...
            if force_target_only:
                ...
                # last-block Q only
                q_tokens = num_frame_per_block * frame_seqlen
                q = q[:, -q_tokens:, :, :]

                q_grid_sizes = torch.tensor(
                    [num_frame_per_block, grid_sizes[1].item(), grid_sizes[2].item()],
                    dtype=grid_sizes.dtype,
                    device=q.device,
                )
                q_start_frame = current_start_frame + (total_num_frames - num_frame_per_block)

                roped_query = causal_rope_apply(
                    q, q_grid_sizes, freqs, start_frame=q_start_frame
                ).type_as(v)

                # print("attn query:", roped_query.shape)
                # print("attn key:", roped_key.shape)
                # print("attn value:", v.shape)
                # V should align with full-window K
                x = attention(
                    roped_query,
                    roped_key,
                    v
                ...
                x = flex_attention(#flex_attention要求长度 pad 到 128 的倍数
            ...
            #输入只有一帧的时候：start=0（max=0)， end=2640，q/k/v都是[1, 2640, 12, 128]、
            #输入history不止一帧， max_attention_size=3520, q: torch.Size([1, 2640, 12, 128]), k/v: torch.Size([1, 3520, 12, 128])
...
class CausalWanAttentionBlock(nn.Module):
    ...
    # 输入：
    # x: [B, L, C]
    # e: [B, F, 6, C]，time modulation
    # context: visual context
    # mouse_cond, keyboard_cond
    # 各种 block mask 和 cache
...
# 把时间步 embedding 切成 6 份，用在：
    # self-attn 输入前的 scale/shift
    # self-attn 输出后的 gate
    # FFN 输入前的 scale/shift
    # FFN 输出后的 gate        
        e = (self.modulation.unsqueeze(1) + e).chunk(6, dim=2)

        # print("block input x:", x.shape)
        # # print("e:", e.shape)
        # print("num_frames:", num_frames, "frame_seqlen:", frame_seqlen)
        # print("context:", context.shape)
        # if mouse_cond is not None:
        #     print("mouse_cond:", mouse_cond.shape)
        # if keyboard_cond is not None:
        #     print("keyboard_cond:", keyboard_cond.shape)

        y = self.self_attn(
            (self.norm1(x).unflatten(dim=1, sizes=(num_frames, frame_seqlen)) * (1 + e[1]) + e[0]).flatten(1, 2),
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, cache_start, force_target_only=force_target_only, num_frame_per_block=num_frame_per_block,)

#self-attn 之后回到主残差流
# 说明 self-attn 输出不是硬替换，而是：
    # 先乘一个 time-step gate e[2]
    # 再 residual add 回 x
    # 所以这是标准 gated residual：       
        # x = x + (y.unflatten(dim=1, sizes=(num_frames, frame_seqlen)) * e[2]).flatten(1, 2)
        if force_target_only:
            target_tokens = num_frame_per_block * frame_seqlen
            x_target = x[:, -target_tokens:]
            e2_target = e[2][:, -num_frame_per_block:]
            x_target = x_target + (y.unflatten(dim=1, sizes=(num_frame_per_block, frame_seqlen)) * e2_target).flatten(1, 2)
            x = torch.cat([x[:, :-target_tokens], x_target], dim=1)
        else:
            x = x + (y.unflatten(dim=1, sizes=(num_frames, frame_seqlen)) * e[2]).flatten(1, 2)
        ...
        def cross_attn_ffn(
            ...
            if force_target_only:
                target_tokens = num_frame_per_block * frame_seqlen
                history_tokens = x.shape[1] - target_tokens

                x_hist = x[:, :history_tokens]
                x_tgt = x[:, history_tokens:]

                # 只对 target 做 cross-attn
                x_tgt = x_tgt + self.cross_attn(
                    self.norm3(x_tgt.to(context.dtype)),
                    context,
                ...
                if self.action_model is not None:
                    ...
                    x_tgt = self.action_model(
                        x_tgt.to(context.dtype),
                        num_frame_per_block,          # tt = 3
                        grid_sizes[1],
                        grid_sizes[2],
                        mouse_cond,
                        keyboard_cond,
                        ...
                        start_frame=start_frame + (num_frames - num_frame_per_block),   # target absolute start
                ...
                # 只对 target 做 FFN
                e3_t = e[3][:, -num_frame_per_block:]
                e4_t = e[4][:, -num_frame_per_block:]
                e5_t = e[5][:, -num_frame_per_block:]

                y_t = self.ffn(
                    (
                        self.norm2(x_tgt)
                        .unflatten(dim=1, sizes=(num_frame_per_block, frame_seqlen))
                        * (1 + e4_t) + e3_t
                    ).flatten(1, 2)
                )

                x_tgt = x_tgt + (
                    y_t.unflatten(dim=1, sizes=(num_frame_per_block, frame_seqlen)) * e5_t
                ).flatten(1, 2)

                # 拼回 4 帧
                x = torch.cat([x_hist, x_tgt], dim=1)
                return x
...
class CausalWanModel(ModelMixin, ConfigMixin, FromOriginalModelMixin, PeftAdapterMixin):
    ...
    def _forward_inference(
        ...
        # print("_forward_inference, input x:", x.shape)
        # print("_forward_inference, cond_concat:", cond_concat.shape)
        # print("forward x before cat: ", x.shape) #[1, 16, 3, 44, 80] 当前 noisy latent block(3 个frame)
    
#cond_concat: [1, 20, 3, 44, 80]一个 20 通道、和当前 target latent block 在时空尺寸上对齐的条件张量。
# 只有一帧 history，也就是第一次生成：
    # cond_concat[:, :, 0] = [mask=1, image_latent=真实首帧latent]
    # cond_concat[:, :, 1] = [mask=0, image_latent≈0-padding encode后的东西]
    # cond_concat[:, :, 2] = [mask=0, image_latent≈0-padding encode后的东西]
# 到后面block的生成：cond_concat 仍然存在，但它已经不再携带“这个 block 内有真实首帧 anchor”的信息了。 因为 mask 全是 0
    # cond_concat[:, :, 6] = [mask=0, zero-like latent]
    # cond_concat[:, :, 7] = [mask=0, zero-like latent]
    # cond_concat[:, :, 8] = [mask=0, zero-like latent]

        x = torch.cat([x, cond_concat], dim=1) # B C' F H W 当前 noisy latent x 和首帧条件 cond_concat，直接沿 channel 维拼起来。
        # print("forward x after cat:", x.shape) #[1, 36, 3, 44, 80]
        # embeddings
        x = self.patch_embedding(x)  #拼接后的 latent+condition 一起被 patchify 成 Transformer token。
        # print("after patch_embedding:", x.shape) #[1, 36, 3, 44, 80]→[1, 1536, 3, 22, 40]

        grid_sizes = torch.tensor(x.shape[2:], dtype=torch.long)
        # print("grid_sizes:", grid_sizes)

        ##然后 flatten 成 token 序列时3 * 22 * 40 = 2640
        x = x.flatten(2).transpose(1, 2) # B FHW C' 之后 Transformer 看到的是视频 patch token 序列 

        seq_lens = torch.tensor([u.size(0) for u in x], dtype=torch.long)
        assert seq_lens[0] <= 15 * 1 * 880
        
        e = self.time_embedding( #diffusion timestep 是通过 scale-shift modulation 进入 block 的。
            sinusoidal_embedding_1d(self.freq_dim, t.flatten()).type_as(x))
        e0 = self.time_projection(e).unflatten(
            1, (6, self.dim)).unflatten(dim=0, sizes=t.shape)
        # context
        context_lens = None
        context = self.img_emb(visual_context) #context 就是首帧图像的语义 token, 以 cross-attention 的 K/V context 形式进入每个 block
        # arguments
        kwargs = dict(
            e=e0,
            seq_lens=seq_lens,
    ...
    #         self.block_mask = self._prepare_blockwise_causal_attn_mask( #不是严格逐帧 causal，而是 blockwise causal。也就是如果 num_frame_per_block = 3，那么一个 block 里的 3 帧会共享同一个“可见终点”。
    ...
    def _forward_train(
        ...
        current_start: int = 0,
        cache_start: int = 0,
        windowed_no_cache: bool = False,
    ):
        r"""
        Forward pass through the diffusion model.

        windowed_no_cache=False:
            原始训练逻辑（teacher forcing full sequence）

        windowed_no_cache=True:
            用局部 window 做 no-cache surrogate forward：
            - 只输入局部 [history + target]
            - 不使用 kv cache
            - 但使用 absolute current_start 来做 RoPE 时间偏移
            - attention mask 让整个局部 window 彼此可见（更接近 inference 里“最近4帧窗口”）
        """
        if mouse_cond is not None or keyboard_cond is not None:
        ...
        if self.freqs.device != device:
            self.freqs = self.freqs.to(device)
        # print("forward x shape before cat: ", x.shape) #[1, 16, 4, 44, 80]
        x = torch.cat([x, cond_concat], dim=1)
        # print("forward x shape after cat: ", x.shape)  #[1, 36, 4, 44, 80]
        ...
        # mask construction
        # -----------------------------------
        # 原始训练：按配置里的 num_frame_per_block 做 blockwise causal
        # windowed_no_cache：把这个局部窗口当作一个完整 block，让窗口内彼此都可见
        if windowed_no_cache:
            effective_num_frame_per_block = self.num_frame_per_block #
            effective_local_attn_size = num_frames #窗口内允许互看
        else:
            effective_num_frame_per_block = self.num_frame_per_block
            effective_local_attn_size = self.local_attn_size
        ...
        x = x.flatten(2).transpose(1, 2)
        seq_lens = torch.tensor([u.size(0) for u in x], dtype=torch.long, device=x.device)
        ...
        kwargs = dict(
            ...
            current_start=current_start,
            cache_start=cache_start,
            force_target_only=windowed_no_cache, #new
        )
