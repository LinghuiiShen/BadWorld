...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, cam_emb, t_mod, freqs):
        ...
        if cam_emb is not None:
            ...
            cam_emb = self.cam_encoder(cam_emb)  # [batch, frames, dim] Step 1：降维 / 投影


            cam_emb = cam_emb.unsqueeze(2).unsqueeze(3).repeat(1, 1, 30, 52, 1) # Step 2：广播到所有 token
            cam_emb = rearrange(cam_emb, 'b f h w d -> b (f h w) d') 
            input_x = input_x + cam_emb #Step 3：直接加到 token 上
