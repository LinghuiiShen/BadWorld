...
def parse_args():
    ...
    parser.add_argument("--checkpoint_path", type=str, default="Matrix-Game-2.0/gta_distilled_model/gta_keyboard2dim.safetensors", help="Path to the checkpoint")
    parser.add_argument("--img_path", type=str, default="demo_images/gta_drive/0001.png", help="Path to the image")
...
class InteractiveGameInference:
    def __init__(self, args):
        ...
        self.frame_process = v2.Compose([
            ...
            # v2.Resize(size=(480, 837), antialias=True),
            v2.ToTensor(),
    ...
    def generate_videos(self):
        ...
        # image = self._resizecrop(image, 832, 480)
        image = self.frame_process(image)[None, :, None, :, :].to(dtype=self.weight_dtype, device=self.device) #shape: [1, 3, 1, 352, 640]
        ...
        img_cond = self.vae.encode(img_cond, device=self.device, **tiler_kwargs).to(self.device)#img_cond.shape = [1, 16, num_output_frames, 44, 80]
        #真正有输入图像信息的，其实只有 第 0 个 latent frame, 其它都是padding 0
        mask_cond = torch.ones_like(img_cond)
        mask_cond[:, :, 1:] = 0
        #img_cond 是 16 通道
        #mask_cond[:, :4] 只取 mask 的前 4 个通道 所以一共20个通道，看_forward_inference的时候也会有 
        #cond_concat： [1, 20, T, 44, 80] T在causual_inference.py的cond_current中会取三个（3 frames/block)
        ...
        with torch.no_grad():
            videos = self.pipeline.inference(
                noise=sampled_noise,  #没有把 initial_latent concat进去（输入图片没有被当成“历史 latent 帧”先塞进 output / kv cache。）
